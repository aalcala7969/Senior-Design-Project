...
class Arm_Device(object):
    ...
    def bus_servo_control_array6(self, array, time=1000):
        ...
        # NEW FUNCTION: Read raw high-resolution PWM pulse (900-3100)
    def bus_servo_read(self, id):
        if id < 1 or id > 6:
            print("id must be 1 - 6")
            return None
        try:
        ...
        # --- THE FIX: Yahboom Software/Hardware Mapping ---
        # Yahboom's playback natively inverts joints 2, 3, and 4.
        # It also subtracts 514 from joint 5. 
        # We must perform the exact mathematical opposite during the read!
        
        if id == 2 or id == 3 or id == 4:
            # Symmetrical inversion (e.g., 3000 becomes 1000)
            pos = 3100 - pos + 900
            
        elif id == 5:
            # Add back the arbitrary offset
            pos = pos + 514
